...
class MailingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if start_datetime < now:
            self.add_error(
                "start_datetime", "Дата и время начала не могут быть в прошлом"
            )
        ...
        if end_datetime < now:
            self.add_error(
                "end_datetime", "Дата и время окончания не могут быть в прошлом"
            )
        ...
        if (end_datetime - start_datetime) < min_duration:
            self.add_error(
                "end_datetime",
                "Интервал между началом и окончанием должен быть не менее 5 минут",
            )
